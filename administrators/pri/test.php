...
function send_gcm_notify($reg_id, $message, $messageId) {
    $fields = array(
...
if(isset($_FILES["inputImage"]["name"])){
    if($_FILES["inputImage"]["name"] == ""){
        //Se inserta mensaje
        $sql = "INSERT INTO messages (mensaje, timestamp) VALUES('$message', '$timestamp')";
        ...
        while($row2 = mysqli_fetch_array($res2)){
            ...
            if(send_gcm_notify($reg_id, $message, $lastInsertId) == 1){
        ...
        if($ROOT == "1"){//Ocurrió error al subir la imagen
            ...
            //Se inserta mensaje
            $sql = "INSERT INTO messages (mensaje, timestamp, picture) VALUES('$message', '$timestamp', '$ROOT')";
            $res = mysqli_query($con,$sql);
            $lastInsertId = mysqli_insert_id($con);

            //Se registra y se envia notificación
            while($row2 = mysqli_fetch_array($res2)){
                $reg_id = $row2[1];
                //Se contabiliza el total de notificaciones enviadas y no enviadas
                if(send_gcm_notify($reg_id, $message, $lastInsertId) == 1){
...
function getImageFromForm(){
    ...
    if (in_array($_FILES['inputImage']['type'], $PERMITIDOS) && $_FILES['inputImage']['size'] <= LIMITE * 1024){
        ...
        $finalRoot = "http://www.entuizer.tech/administrators/pri/img_for_messages/" . $_FILES['inputImage']['name'];
