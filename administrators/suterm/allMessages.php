    ...
    <body> 
            ...
            <div class="row">
                <div class="col-md-12" >
                    <table id="addQuery" class="display table table-stripped" style="max-width: none !important;display: none !important;">
                        ...
                        <tbody>
                            <!-- Aquí se incluyen la consulta de los mensajes que se hace a través de AJAX -->
                        </tbody>
                        <tfoot>
                            <tr>
                                <th>#</th>
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                <p></p>
                <button type="button" class="btn btn-success btnDelete" >Eliminar</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">Cancelar</button>
                <input type="hidden" id="inputId"></input>
              </div>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                if($('#ajaxMask:visible')){
                    var addQuery;
                    $.post('webServices/getAllMessages.php', {param1: 'value1'}, function(data, textStatus, xhr) {                        
                        $('tbody').html(data);
                        $('#addQuery').fadeIn(300);
                        
                        //Carga paginación de tabla
                        addQuery = $('#addQuery').DataTable({
                            "language": {
                ...
                //Muestra ventana modal para eliminación de mensaje y envía id del mensaje que se desea eliminar
                $('#addQuery').on('click', '.btnDeleteMessage', function(event) {console.log('eliminar mensaje');
                    event.preventDefault();
                    var messageId = $(this).data('id');
                    $('#inputId').val(messageId);
                    $('.modal-body p').text('¿Está seguro que desea eliminar el mensaje # '+messageId+' ?');
                });

                //Confirma eliminación de mensaje a la base de datos
                $('.btnDelete').click(function(event) {
                    /* Act on the event */
                    $('#ajaxMask').fadeIn(300);
                    console.log('boton eliminar');
                    var messageId = $('#inputId').val();
                    $.post('webServices/deleteMessage.php', {messageId: messageId}, function(data, textStatus, xhr) {
                        $.each(data, function(index, item) {
                             if(item.ERROR == 0){console.log(item.MESSAGE);
                                alert(item.MESSAGE);
                                $('#ajaxMask').fadeOut(300);
                                $('#deleteMessage').modal('hide');
                                location.reload();
                             }else{
                                alert(item.MESSAGE);console.log(item.MESSAGE);
                                $('#ajaxMask').fadeOut(300);
                             }
                        });
                    });
